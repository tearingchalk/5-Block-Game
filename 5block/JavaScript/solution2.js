...
function checkLine(target) {
    let selected = Number(target.textContent);
    ...
    for (let i = 0; i < 4; i++) {
        ...
        switch (i) {
            case 0:
                // board.childNodes[row].childNodes[0].childNodes[col];
                temp.push(nodes[row - 2][col - 2], nodes[row - 1][col - 1], nodes[row][col], nodes[row + 1][col + 1], nodes[row + 2][col + 2]);
                arr.push(temp);
                break;
            case 1:
                temp.push(nodes[row - 2][col], nodes[row - 1][col], nodes[row][col], nodes[row + 1][col], nodes[row + 2][col]);
                arr.push(temp);
                break;
            case 2:
                temp.push(nodes[row][col - 2], nodes[row][col - 1], nodes[row][col], nodes[row][col + 1], nodes[row][col + 2]);
                arr.push(temp);
                break;
            case 3:
                temp.push(nodes[row + 2][col - 2], nodes[row + 1][col - 1], nodes[row][col], nodes[row - 1][col + 1], nodes[row - 2][col + 2]);
